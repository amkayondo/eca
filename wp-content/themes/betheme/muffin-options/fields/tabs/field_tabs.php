...
class MFN_Options_tabs extends Mfn_Options_field
{
	...
	public function render( $existing = false, $vb = false )
	{
					...
					echo '<div class="tab-content">';
						...
						foreach( $this->field['options'] as $label => $param ){
							...
							echo '<div class="form-control">';
								...
								if( 'input' == $param[0] ){
									echo '<input class="not-empty mfn-form-control mfn-form-input mfn-tab-'. esc_attr($label) .' '. esc_attr($js) .'" type="text" data-default="'. esc_attr( $field_name ) .'" value="'. esc_html( $param[2] ) .'"/>';
								} else if ( 'textarea' == $param[0] ){
									echo '<textarea class="not-empty mfn-form-control mfn-form-textarea" rows="3" data-default="'. esc_attr( $field_name ) .'">'. esc_textarea( $param[2] ) .'</textarea>';
								}
								...
								echo '<div class="tab-content">';
									...
									foreach( $this->field['options'] as $label => $param ){
										...
										echo '<div class="form-control">';
											...
											if( 'input' == $param[0] ){
												echo '<input type="text" class="not-empty mfn-form-control mfn-form-input mfn-tab-'. esc_attr( $label ) .' '. esc_attr($js) .'" '. esc_attr( $field_prefix ) .'name="'. esc_attr( $field_name ) .'" value="'. esc_html( $value[$label] ) .'"/>';
											} else if ( 'textarea' == $param[0] ){
