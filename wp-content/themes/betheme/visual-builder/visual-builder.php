...
function mfnvb_importfromclipboard(){
	...
	$type = $_POST['type'];
		...
		$mfnvb = new MfnVisualBuilder();
		if($type == 'column'){
			foreach($mfn_items as $section){ foreach($section['wraps'] as $wrap){ $mfnvb->mfn_createForm_items($wrap['items'], $count, 0, 0, $releaser); } }
		}else if($type == 'wrap'){
			foreach($mfn_items as $section){ $mfnvb->mfn_createForm_wraps($section['wraps'], $count, 0, $releaser); }
		}else{
			$mfnvb->mfn_createForm($mfn_items, $count, $releaser);
		}
		// $mfnvb->mfn_createForm($mfn_items, $count, $releaser);
